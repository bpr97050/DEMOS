/* === CDEBUGGER === ПОКАЗ БАЙТА С ПРЕОБРАЗОВАНИЕМ СПЕЦ-СИМВОЛОВ В \XXX */
...
/* ВВОД БАЙТА ПО УКАЗАТЕЛЮ 'pcmn' С УЧЕТОМ \ СИМВОЛОВ */
...
char get_char()
{
    ...
    if( (c = *pcmn++) == '\\' ) {
        ...
        if( c>='0' && c<='9' ) {
            ...
            if( i<0 || i>=377 ) error( 17 );    /* неверное \000 число */
            return( i );
        }
    }
    return( c );
...
/* ВВОД ОДНОГО БАЙТА ПО АДРЕСУ 'adr' */
...
/* ВЫВОД БАЙТА 'value' ПО АДРЕСУ 'adr' */
